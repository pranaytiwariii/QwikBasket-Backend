...
export const getSubCategories=async(req,res)=>{
    try {
        const subcategories=await SubCategory.find({}).populate('parentCategory','name image').lean();
        res.status(200).json({
            success: true,
            data: subcategories
          });
...
export const getSubCategoryById=async(req,res)=>{
    try {
        const {id}=req.params;
        const subcategory = await SubCategory.findById(id).populate('parentCategory', 'name image').lean();
